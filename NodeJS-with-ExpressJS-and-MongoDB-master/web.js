...
app.get('/testembed', function(req, res){
  ...
  BlogPost.findById('4f55bf0ad579ec0100000002', function(err, blogpost){
    ...
    var comment = blogpost.comments.id('4f67b90776dd2e010000001a');
...
app.get('/data/allposts', function(request, response){
  ...
  var includeFields = ['title','content','urlslug','date','comments','author.name'];

    // query for all blog
  var queryConditions = {}; //empty conditions - return everything
...
app.get('/jsontest',function(request, response) {
  ...
  var blogPostsURL= 'http://dwd-mongodb.herokuapp.com/data/allposts'; //pretend this url is actually on another server
  ...
  requestURL(blogPostsURL, function(error, httpResponse, data) {
    ...
    if (httpResponse.statusCode === 200) {
      ...
      var blogPostData = JSON.parse(data);

      if (blogPostData.status === 'OK') {
        var posts = blogPostData.posts;
...
app.get('/weather/:location', function(request, response){
  ...
  var requestedLocation = request.params.location.toLowerCase();

    // lookup the location in YAHOOLocations
  if (requestedLocation in YAHOOLocations ) {
    woeid = YAHOOLocations[requestedLocation];
  } else {
    var woeid = YAHOOLocations['nyc']; // default to nyc
  }

    // build the request URL
  var yahooWeatherURL = 'http://weather.yahooapis.com/forecastjson?w=' + woeid;
